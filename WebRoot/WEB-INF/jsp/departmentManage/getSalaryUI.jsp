  ...
  <head>
	...
	<script>
		var formCtrl = ['$scope','$http',function($scope,$http){
			...
			//修改个人工资
			$scope.edits = function(salary){
				$http.get('./'+salary.id+'/edits',{params:salary}).success(function(){
					$http.get('./getAvg').success(function(data){
						//更新平均工资
						$scope.avg = data;
					});
				});
			...
			$scope.print = function(name){
				var departmentName = encodeURI(encodeURI(name));
				window.location.href = "./print/"+departmentName;
			}
			//总收入
  ...
  <body >
  	...
  	<div class="container" ng-controller="formCtrl">
	    ...
	    <table class="table table-condensed table-striped">
	    	<caption><h2>{{department.name}}工资汇总表</h2></caption>
	    	<tr>
	    		<th>员工编号</th>
	    		<th>员工姓名</th>
	    		<th>总收入</th>
	    	...
	    	<tr ng-repeat="list in pager.recordList">
	    		...
	    		<td>
	    			<a href="javascript:void(0);" ng-click="edits(list.salary)" class="btn">
		...
		<div class="row">
			 <a href="javascript: window.history.back(-1);" class="btn btn-primary pull-right span2">
				返回部门列表
			</a>
			
			<a href="javascript:void(0);" ng-click="print(department.name)" class="btn btn-primary pull-right span2">
